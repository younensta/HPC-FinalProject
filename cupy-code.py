...
def main():
    ...
    # Domain setup
    L = 1    
    xlin = cp.linspace(0, L, num=N+1)[:-1]  # Remove duplicate point
    ...
    klin = 2.0 * cp.pi / L * cp.fft.fftfreq(N, d=1/N)
    kx, ky = cp.meshgrid(klin, klin)
    kSq = kx**2 + ky**2
    kSq_inv = cp.zeros_like(kSq)
    kSq_inv[kSq > 0] = 1.0 / kSq[kSq > 0]
    
    dealias = (cp.abs(kx) < (2./3.)*cp.max(klin)) & (cp.abs(ky) < (2./3.)*cp.max(klin))
    
    # Transform initial condition to Fourier space
    vx_hat = cp.fft.fftn(vx)
    vy_hat = cp.fft.fftn(vy)
    ...
    for i in range(Nt):
        ...
        ifft_vx = cp.real(cp.fft.ifftn(vx_hat))
        ifft_vy = cp.real(cp.fft.ifftn(vy_hat))

        rhs_x_hat = -cp.fft.fftn(ifft_vx * cp.real(cp.fft.ifftn(dvx_x_hat)) +
                                 ifft_vy * cp.real(cp.fft.ifftn(dvx_y_hat)))
        
        rhs_y_hat = -cp.fft.fftn(ifft_vx * cp.real(cp.fft.ifftn(dvy_x_hat)) +
                                 ifft_vy * cp.real(cp.fft.ifftn(dvy_y_hat)))
        ...
        if plotRealTime or (i == Nt - 1):
            ...
            wz = cp.real(cp.fft.ifftn(wz_hat))
        ...
        if (plotRealTime and plotThisTurn) or (i == Nt - 1):
            plt.cla()
            wz = wz.get() #To turn into a numpy array
