...
def main():
    ...
    vx_hat = cfft.fftn_cython(vx)
    vy_hat = cfft.fftn_cython(vy)
    ...
    for i in range(Nt):
        ...
        ifft_vx = np.real(cfft.ifftn_cython(vx_hat))
        ifft_vy = np.real(cfft.ifftn_cython(vy_hat))

        rhs_x_hat = -cfft.fftn_cython(ifft_vx * np.real(cfft.ifftn_cython(dvx_x_hat)) +
                                 ifft_vy * np.real(cfft.ifftn_cython(dvx_y_hat)))
        
        rhs_y_hat = -cfft.fftn_cython(ifft_vx * np.real(cfft.ifftn_cython(dvy_x_hat)) +
                                 ifft_vy * np.real(cfft.ifftn_cython(dvy_y_hat)))
        ...
        if plotRealTime or (i == Nt - 1):
            ...
            wz = np.real(cfft.ifftn_cython(wz_hat))
